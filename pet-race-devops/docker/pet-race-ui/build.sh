...
docker exec jhipster /bin/bash -c 'git clone https://github.com/k8s-for-greeks/gpmr.git . && cd pet-race-ui && npm install && npm install statsjs && bower install && ./gradlew bootRepackage -PskipTest -Pprod -x test -x gulp_test && mv build/libs/*.war ~/app.war'
